...
/* (10) sprawdzenie czy menu jest otwarte */
bool menu_is_open(void) {
    return menu_running;
}

/* (11) główna funkcja obsługi menu */
menu_status_t menu_update(surface_t *disp, joypad_buttons_t pressed, joypad_buttons_t held, const resolution_t **out_selected) {

    // (11.1) sprawdzenie czy menu działa
    if (!menu_running) {
    ...
    // (11.2) wymiary i pozycjonowanie boxa
    ...
    // (11.3) obsługa nawigacji góra/dół + powtarzanie
    if (pressed.d_up) {
    ...
    // (11.8) rysowanie tła i ramki
    uint32_t frame_col = graphics_make_color(0,200,0,255);
    graphics_draw_box(disp, box_x, box_y, box_w, box_h, graphics_make_color(24,24,24,200));
    graphics_draw_line(disp, box_x - 2, box_y - 2, box_x + box_w + 1, box_y - 2, frame_col);
    graphics_draw_line(disp, box_x - 2, box_y + box_h + 1, box_x + box_w + 1, box_y + box_h + 1, frame_col);
    graphics_draw_line(disp, box_x - 2, box_y - 2, box_x - 2, box_y + box_h + 1, frame_col);
    graphics_draw_line(disp, box_x + box_w + 1, box_y - 2, box_x + box_w + 1, box_y + box_h + 1, frame_col);
    ...
    for (int i = 0; i < visible_lines; ++i) {
        ...
        if (idx == menu_selected) {
            graphics_draw_box(disp, box_x + pad_x - 2, y - 2, box_w - pad_x*2 + 4, line_h + 2,
                              graphics_make_color(200,200,0,255));
